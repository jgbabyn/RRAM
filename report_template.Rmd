...
syears = unlist(lapply(tmb.data$survey_list,function(x){x$year+1}))
survey15y = seq(tmb.data$start_year,tmb.data$end_year)[syears]
tot_N = extract_from_sdr(outdat$ssdr,'log_total_survey_abundance',year=survey15y,flatten=TRUE) 
...
syears = unlist(lapply(tmb.data$survey_list,function(x){x$year+1}))
survey15y = seq(tmb.data$start_year,tmb.data$end_year)[syears]
survey15 = extract_from_sdr(ssdr,"log_survey15",year=survey15y,flatten=TRUE)
...
```{r echo = FALSE,message=FALSE,warning=FALSE,out.width="100%"}
  landings = extract_from_sdr(ssdr,'log_expected_landings',year=tmb.data$start_year:tmb.data$end_year,flatten=TRUE)
...
```{r echo = FALSE,message=FALSE,warning=FALSE,out.width="100%"}
  ...
  QRho$prior = tmb.data$Qpriors[1:tmb.data$end_length]
  ...
  QRhop =  basic_plot(x=QRho$length,y=QRho$prior,ymin=NULL,ymax=NULL,plot_group='Prior',color='blue',title='Ratio of Survey Qs',xlab='Length',ylab='$\\frac{q_{l,E}}{q_{l,C}}$',add=QRhop)
